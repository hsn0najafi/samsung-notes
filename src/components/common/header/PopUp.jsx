...
const PopUp = () => {
  ...
  return (
    <div>
      {c.showCategoriPopUp ? (
        <div className="popUpTop flex-center-col" onClick={c.handleSetShowCategoriPopUp}>
          <input
            placeholder="اسم جدید رو وارد کن ..."
            className="animate__animated animate__fadeIn"
            value={c.newCategoriTitle}
            style={c.categoriEditMode && c.showCategoriNameInput ? null : { display: 'none' }}
      ...
      >
        {c.categories.map((categori) => (
          ...
          >
            <p>{categori.name}</p>
            <div style={c.categoriEditMode ? null : { display: 'none' }} className="categoriEditorTools flex-center-row">
              <i className="fa fa-trash" onClick={() => c.handleDeleteCategori(categori.id)}></i>
              <i
                onClick={() => c.handleCategoriNameChange(categori.id)}
                className={c.showCategoriNameInput ? 'fa fa-check' : 'fa fa-pen'}
              ></i>
            </div>
            <p style={{ width: '3rem', alignItems: 'center' }}>{categori.count}</p>
          </div>
        ))}
        <div
        ...
        >
          <p>Edit Catrgories</p>
          <p style={{ width: '5rem', alignItems: 'center' }} className="addNewCategoriButton flex-center-col">
